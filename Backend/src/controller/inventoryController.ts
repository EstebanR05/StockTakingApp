...
export const getAllInventory = async (_req: Request, res: Response) => {
    try {
        const result: IInventory[] = await findAllInventories((_req as any).idUser);
...
export const getByIdInventory = async (_req: Request, res: Response) => { 
    try {
        const result: IInventory = await findByIdInventories(parseInt(_req.params.id, 10), (_req as any).idUser);
        res.status(200).send(result);
    } catch (error: any) {
        res.status(500).json({ message: error.error });
    }
